...
Hemlock favors contiguous allocation over segregated allocation, and it
...
Many aspects of global heap management rely on a global monotonic clock. Precise
...
- The major heap comprises two distinct memory-limit-sized virtual memory
...
The major heap comprises two semispaces so that copy-compaction can be performed
on a per page incremental basis rather than monolithically. Upon completion of
marking the tospace is initialized with its pages inaccessible. Each tospace
page is made accessible as soon as it is copy-compacted, whether due to an
incremental sweep or on demand (i.e. a `SIGSEGV` signal triggered by actor
execution). Preservation of allocation order during copy-compaction allows
fromspace pages to be incrementally deallocated when all their reachable values
have been copied to the tospace. Therefore the peak major heap memory usage is
approximately bounded by the heap size limit.
...
Otherwise the virtual memory range is utilized as a ring buffer. In the event
that a message cannot be delivered because the end of the virtual memory range
is too close, the frontier wraps around to the beginning of the virtual memory
range. As a consequence, there may be unusable pad space that is nearly as large
as the message which triggered wrapping. In principle this can cause the heap
size limit to be exceeded when the available mailbox space is nearly twice as
much as the message which triggers failure, but in practice this is not a
concern because messages are almost always small relative to the mailbox size.
...
The executor allocates virtual memory areas (VMAs) in exponentially increasing
sizes and sub-allocates the actor heaps, so that even with millions of actors,
the total number of VMAs is modest. In the following figure, four local heaps
are suballocated from one VMA pair. In the diagram the actors all have the same
memory limit, but that need not be true in general.
