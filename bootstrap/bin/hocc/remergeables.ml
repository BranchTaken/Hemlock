...
let pp_core_rels {mergeable_sets; issn2statenub; _} formatter =
  let _, StateNub.{isocores_sn; _} = Map.choose_hlt issn2statenub in
  formatter
  |> Fmt.fmt "{"
  |> fun formatter -> List.fold ~init:formatter ~f:(fun formatter mergeable_set ->
    Bitset.foldi ~init:formatter ~f:(fun i formatter issn ->
      let statenub = Map.get_hlt issn issn2statenub in
      let index = StateNub.index statenub in
      formatter
      |> Fmt.fmt (match i with 0L -> "" | _ -> ",")
      |> StateNub.Index.pp index
    ) mergeable_set
  ) mergeable_sets
  |> Fmt.fmt "}"
  |> Fmt.fmt " [" |> Uns.pp isocores_sn |> Fmt.fmt ".{"
  |> fun formatter -> List.fold ~init:formatter ~f:(fun formatter mergeable_set ->
    Bitset.foldi ~init:formatter ~f:(fun i formatter issn ->
      formatter
      |> Fmt.fmt (match i with 0L -> "" | _ -> ",")
...
let fmt ?(alt=false) ?(width=0L) {cores_map; _} formatter =
  let ord_core_rels =
    Map.fold ~init:(Ordmap.empty (module Uns))
      ~f:(fun ord_core_rels (_core, ({issn2statenub; _} as core_rels)) ->
        match Map.choose issn2statenub with
        | None -> ord_core_rels
        | Some (_, StateNub.{isocores_sn; _}) ->
          Ordmap.insert_hlt ~k:isocores_sn ~v:core_rels ord_core_rels
      ) cores_map
    |> Ordmap.to_array
    |> Array.map ~f:(fun (_core, core_rels) -> core_rels)
    |> Array.to_list
  in
  formatter
  |> List.fmt ~alt ~width pp_core_rels ord_core_rels
