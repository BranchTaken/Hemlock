...
module T = struct
  ...
  let hash_fold {symbol_index; conflict; isucc_lr1itemset; contrib} state =
    state
    |> Uns.hash_fold 1L |> Symbol.Index.hash_fold symbol_index
    |> Uns.hash_fold 2L |> Contrib.hash_fold conflict
    |> Uns.hash_fold 2L |> Lr0Itemset.hash_fold (Lr1Itemset.core isucc_lr1itemset)
    |> Uns.hash_fold 3L |> Contrib.hash_fold contrib

  let cmp {symbol_index=s0; conflict=x0; isucc_lr1itemset=is0; contrib=c0}
    {symbol_index=s1; conflict=x1; isucc_lr1itemset=is1; contrib=c1} =
    let open Cmp in
  ...
  let equal {symbol_index=s0; conflict=x0; isucc_lr1itemset=is0; contrib=c0}
    {symbol_index=s1; conflict=x1; isucc_lr1itemset=is1; contrib=c1} =
  ...
  let pp {symbol_index; conflict; isucc_lr1itemset; contrib} formatter =
    formatter
  ...
  let fmt_hr symbols prods ?(alt=false) ?(width=0L)
    {symbol_index; conflict; isucc_lr1itemset; contrib} formatter =
    formatter
  ...
  let empty ~symbol_index ~conflict =
    {symbol_index; conflict; isucc_lr1itemset=Lr1Itemset.empty; contrib=Contrib.empty}

  let init ~symbol_index ~conflict ~isucc_lr1itemset ~contrib =
    {symbol_index; conflict; isucc_lr1itemset; contrib}

  let is_empty {contrib; _} =
    Contrib.is_empty contrib

  let union {symbol_index=s0; conflict=x0; isucc_lr1itemset=is0; contrib=c0}
    {symbol_index=s1; conflict=x1; isucc_lr1itemset=is1; contrib=c1} =
    assert Symbol.Index.(s0 = s1);
    assert Contrib.(x0 = x1);
    init ~symbol_index:s0 ~conflict:x0 ~isucc_lr1itemset:(Lr1Itemset.union is0 is1)
      ~contrib:(Contrib.union c0 c1)

  let inter {symbol_index=s0; conflict=x0; isucc_lr1itemset=is0; contrib=c0}
    {symbol_index=s1; conflict=x1; isucc_lr1itemset=is1; contrib=c1} =
    assert Symbol.Index.(s0 = s1);
    assert Contrib.(x0 = x1);
    init ~symbol_index:s0 ~conflict:x0 ~isucc_lr1itemset:(Lr1Itemset.inter is0 is1)
