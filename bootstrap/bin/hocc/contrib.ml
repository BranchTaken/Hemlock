...
let compat_ielr1 ~resolve:do_resolve symbols prods symbol_index t0 t1 =
   ...
   * lane. *)
  let r0, r1 = match do_resolve with
    | false -> t0, t1
    | true -> (resolve symbols prods symbol_index t0), (resolve symbols prods symbol_index t1)
  in
  (* Determine compatibility. *)
  let ret =
    match length r0, length r1 with
    | 0L, 0L -> begin
  ...
  in
(*
  File.Fmt.stderr |> Fmt.fmt "XXX Contrib.compat_ielr1 symbol_index=" |> Symbol.Index.pp symbol_index |> Fmt.fmt " -> " |> Bool.pp ret |> Fmt.fmt "\n" |> ignore;
  File.Fmt.stderr |> Fmt.fmt "             t0=" |> pp t0 |> Fmt.fmt "\n" |> ignore;
  File.Fmt.stderr |> Fmt.fmt "             t1=" |> pp t1 |> Fmt.fmt "\n" |> ignore;
  File.Fmt.stderr |> Fmt.fmt "             r0=" |> pp r0 |> Fmt.fmt "\n" |> ignore;
  File.Fmt.stderr |> Fmt.fmt "             r1=" |> pp r1 |> Fmt.fmt "\n" |> ignore;
*)
  ret
