      ...
      ~f:(fun (formatter, first_line)
        ...
        |> (fun formatter ->
          ...
          |> (fun formatter ->
            ...
            | _ -> begin
                formatter
                |> Fmt.fmt "(Bitset.of_list "
                |> List.fmt Symbol.Index.pp (Bitset.to_list first)
                |> Fmt.fmt ")"
              end
          )
          ...
          |> (fun formatter ->
            ...
            | _ -> begin
                formatter
                |> Fmt.fmt "(Bitset.of_list "
                |> List.pp Symbol.Index.pp (Bitset.to_list follow)
                |> Fmt.fmt ")"
              end
          )
        ),
          ...
          ~f:(fun formatter {lr0item={prod={index=prod_index; _}; dot}; follow} ->
            ...
            |> (fun formatter ->
              ...
              |> (fun formatter ->
                ...
                let indent = mk_indent indentation in
                formatter
                |> indent |> Fmt.fmt "Bitset.of_list " |> Bitset.pp follow |> Fmt.fmt "\n"
              )
        ...
        |> (fun formatter ->
          ...
          |> (fun formatter ->
            ...
            | _ -> begin
                formatter
                |> Fmt.fmt "(Bitset.of_list "
                |> List.fmt ml_uns_pp (Bitset.to_list first)
                |> Fmt.fmt ")"
              end
          )
          |> Fmt.fmt "\n" |> indent |> Fmt.fmt "  ~follow:"
          |> (fun formatter ->
            ...
            | _ -> begin
                formatter
                |> Fmt.fmt "(Bitset.of_list "
                |> List.pp ml_uns_pp (Bitset.to_list follow)
                |> Fmt.fmt ")"
              end
          )
        ),
          ...
          ~f:(fun formatter {lr0item={prod={index=prod_index; _}; dot}; follow} ->
            ...
            |> (fun formatter ->
              ...
              |> (fun formatter ->
                ...
                let indent = mk_indent indentation in
                formatter
                |> indent |> Fmt.fmt "Bitset.of_list "
                |> (Bitset.to_list follow |> List.pp ml_uns_pp)
                |> Fmt.fmt "\n"
              )
              |> indent |> Fmt.fmt "  ) in\n"
              |> indent |> Fmt.fmt "lr0item, lr1item\n"
