...
let generate_hmi conf Parse.(Hmhi {prelude; hocc; postlude; eoi=Eoi {eoi}}) io spec =
  ...
  let io =
    ...
    |> Fmt.fmt " rather than "
    |> Fmt.fmt (String.to_string ~pretty:true hmi_name)
    |> Fmt.fmt "\n"
...
let expand_hm_reductions hocc_block reductions ~indentation formatter =
...
let expand_hm_template template_indentation template hocc_block
  ...
  let expanders = Map.of_alist (module String) [
    ...
    ("«reductions»", expand_hm_reductions hocc_block reductions);
