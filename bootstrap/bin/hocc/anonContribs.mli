...
module Attrib : sig
  type t = {
    ...
    conflict: Contrib.t;
    (** Conflict on symbol. This is a non-strict superset of attributed conflict contribution,
        i.e. the attribution may not explain the entire conflict. *)
  ...
  (** [pp_hr symbols prods t formatter] formats a human-readable representation of [t]. *)

  val empty: conflict_state_index:StateIndex.t -> symbol_index:Symbol.Index.t -> conflict:Contrib.t
    -> t
  (** [empty ~conflict_state_index ~symbol_index ~conflict] returns an empty attribution, i.e. with
      no conflict contributions. *)
  ...
  (** [init ~conflict_state_index ~symbol_index ~conflict ~contrib] returns an attribution with key
      (conflict_state_index, symbol_index) that attributes [contrib]. *)

  val is_empty: t -> bool
  (** [is_empty t] returns true if there are no attributions in [t]. *)

  val union: t -> t -> t
  (** [union t0 t1] returns an attribution with the union of attribution values in [t0] and [t1].
...
val length: t -> uns
(** [length t] returns the number of distinct attrib values in [t]. *)
...
(** [insert attrib t] inserts the attribution [attrib] to state [attrib.conflict_state_index] on
    symbol [attrib.symbol_index]. *)
...
(** [fold ~init ~f t] folds over the attrib values in [t], using [init] as the initial accumulator value. *)
...
(** [fold2_until ~init ~f t0 t1] folds over the attrib values in [t0] and [t1]. Attrib values with
    equal (conflict state index, symbol_index) are paired in calls to [~f]; tuples missing from one
...
(** [fold2 ~init ~f t0 t1] folds over the attrib values in [t0] and [t1]. Attrib values with equal
    (conflict state index, symbol_index) are paired in calls to [~f]; tuples missing from one of the
