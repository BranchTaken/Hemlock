(** Collection of distinct (conflict state, symbol, contrib) tuples. *)
...
(** [amend ~conflict_state_index symbol_index ~f t] returns an incremental derivative of [t] that is
    ...
    [symbol_index]}, as determined by the result of [~f contrib_opt], where [contrib_opt = Some contrib]
    indicates [symbol_index] is associated with [contrib] in [t], and [contrib_opt = None] indicates
    [symbol_index] is not attributed in [t]. The result contains a mapping from [symbol_index] to
    [contrib'] if [~f contrib_opt] returns [Some contrib']; the result contains no attribution for
    [symbol_index] if [~f contrib_opt] returns [None]. *)
...
(** [fold ~init ~f t] folds over the (conflict state index, symbol index, contrib) tuples in [t], using
    [init] as the initial accumulator value, continuing until [f] returns [accum, true], or until
    folding is complete if [f] always returns [accum, false]. *)

val fold: init:'accum -> f:('accum -> StateIndex.t -> Symbol.Index.t -> Contrib.t -> 'accum) -> t
  -> 'accum
(** [fold ~init ~f t] folds over the (conflict state index, symbol index, contrib) tuples in [t], using
    [init] as the initial accumulator value. The conflict state index is invariant if combined with
    [filter], e.g. [fold ~init ~f (filter ~conflict_state_index t)]. *)

val fold2_until: init:'accum -> f:('accum -> StateIndex.t -> Symbol.Index.t -> Contrib.t option
  -> Contrib.t option -> 'accum * bool) -> t -> t -> 'accum
(** [fold2_until ~init ~f t0 t1] folds over the (conflict state index, symbol index, contrib) tuples in
    [t0] and [t1]. Tuples that differ only in the contrib component are paired in calls to [~f]; tuples
...
(** [fold2 ~init ~f t0 t1] folds over the (conflict state index, symbol index, contrib) tuples in [t0]
    and [t1]. Tuples that differ only in the contrib component are paired in calls to [~f]; tuples
    missing from one of the inputs are signified by [None]. *)
