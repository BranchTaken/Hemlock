(** Functor interfaces and signatures for integers of optionally constrained
    wordwidth. These functors provide the building blocks for the
    fixed-wordwidth types ({!type:u128}, {!type:u256}, {!type:u512},
    {!type:i128}, {!type:i256}, and {!type:i512}), but arbitrary-precision
    integers are also supported via relaxed wordwidth constraints
    (\[0..Uns.max_value\]). *)
...
module type IV = sig
  ...
  (** Minimum number of 64-bit words in physical representation, at least 1, and
      no larger than {!max_word_length}. *)

  val max_word_length: uns
  (** Maximum number of 64-bit words in physical representation, no smaller than
      {!min_word_length}. *)

  val init: uns -> f:(uns -> u64) -> t
  (** Initialize integer. [init len ~f] initializes an integer of given
      wordwidth, where [f] provides the value for each element at given index.
  *)

  val word_length: t -> uns
  (** Current number of 64-bit words in physical representation, in
...
module type IF = sig
  ...
  (** Initialize integer. [init len ~f] initializes an integer of fixed
      wordwidth, where [f] provides the value for each element at given index.
  *)
...
module type SCommon = sig
  ...
  (** Convert to 64-bit unsigned integer, or return [None] if conversion would
      be lossy. *)
...
module type SSigned = sig
  ...
  (** Convert to 64-bit signed integer, or return [None] if conversion would be
      lossy. *)
  ...
  (** Convert to default-width signed integer, or return [None] if conversion
      would be lossy. *)

  val to_sint_hlt: t -> sint
  (** Convert to default-width signed integer, or halt if conversion would be
      lossy. *)
...
module type SVCommon = sig
  ...
  (** Number of bits in physical representation, equivalent to [word_length t *
      64]. *)
end

(** Functor output signature for an unsigned integer type with a variable
    wordwidth. *)
module type SVU = sig
  type t
...
(** Functor output signature for a signed integer type with a variable
    wordwidth. *)
module type SVI = sig
  type t
...
(** Functor output signature for an unsigned integer type with a fixed
    wordwidth. *)
module type SFU = sig
  type t
...
(** Functor output signature for a signed integer type with a fixed
    wordwidth. *)
module type SFI = sig
  type t
