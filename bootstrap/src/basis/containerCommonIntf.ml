...
(** {!module:SPolyFoldGen} is equivalent to {!module:SPolyFold}, except that {!type:'a elm} is
    explicit. This near-identical signature exists exclusively to enable functor implementation. *)
...
(** General functor input interface for polymorphic containers, e.g. {!type:('a, 'cmp) Ordset}. *)
...
(** General functor input interface for polymorphic containers, e.g. {!type:('k, 'v, 'cmp) Ordmap}.
*)
