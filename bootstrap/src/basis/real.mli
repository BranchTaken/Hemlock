...
(** [create ~neg ~exponent ~mantissa] creates a real, where [-1023 <= exponent <= 1024] and
...
val mantissa: t -> uns
(** [mantissa t] returns the mantissa of [t], which is in [\[0 .. 0xf_ffff_ffff_ffff\]]. *)

val m2x: t -> t * sint
(** [m2x t] splits [t] into a normalized fraction [f] and exponent [x], where [t = f*2^x]. *)

val f2x: p:sint -> t -> t
(** [f2x ~p t] computes [t*2^~p]. *)

val modf: t -> Parts.t
(** [modf t] splits decomposes [t] into the fractional value and integral value which sum to [t].
    See {!module:Parts} for further detail. *)
...
(** [copysign ~sign t] returns the composition of the absolute value of [t] and the sign of [~sign].
*)
...
(** [round ~dir t] rounds [t] in the specified direction. [dir] defaults to [Nearest]. *)
...
(** [lngamma t] computes the natural log of the {{: https://en.wikipedia.org/wiki/Gamma_function}
    ...
    Pike, M.C., I.D. Hill (1966) Algorithm 291: Logarithm of Gamma function \[S14\]. Communications
    of the ACM 9(9):684. *)

val gamma: t -> t
(** [gamma t] applies the {{: https://en.wikipedia.org/wiki/Gamma_function} Gamma} function to [t].
*)
...
val hypot: t -> t -> t
(** [hypot t0 t1] computes the hypotneuse length for a right triangle with sides of length [t0] and
    [t1], i.e [sqrt (t0 * t0 + t1 * t1)]. *)
...
val atan2: t -> t -> t
(** [atan2 t0 t1] computes the arc tangent of [t0 / t1], where the signs of [t1] and [t0] determine
