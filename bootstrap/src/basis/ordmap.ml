...
module T = struct
  ...
  module Cursor = struct
    module T = struct
      ...
      let seek i t =
        match Int.cmp i (Int.kv 0) with
        | Lt -> begin
            let u = (Uns.of_int Int.(neg i)) in
  ...
  let test_fwd ordmap = begin
    let rec fn cursor = begin
      ...
      | false -> begin
          ...
          assert Cursor.((seek (Uns.to_int i) (hd ordmap)) = cursor);
          printf "            %a=%a@\n"
            Cursor.pp cursor
