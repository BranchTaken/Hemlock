...
module type IMono = sig
  ...
  val cmp: t -> t -> Cmp.t
  (** [cmp a b] returns [Cmp.lt] if [a < b], [Cmp.eq] if [a = b], or [Cmp.gt] if [a > b]. *)
end
...
(** Functor output signature for infix comparisons on comparable monomorphic types. *)
...
module type SMono = sig
  ...
  (** [ascending t0 t1] compares [t0] and [t1] in ascending order. Equivalent to [cmp t0 t1]. *)

  val descending: t -> t -> Cmp.t
  (** [descending t0 t1] compares [t0] and [t1] in descending order. If [cmp] provides total
      ordering, this is equivalent to [cmp t1 t0], but the implementation does not assume [cmp] is
      implemented as such. *)

  val clamp: min:t -> max:t -> t -> t
  (** [clamp ~min ~max t] returns [t] unless it is outside the open-open range [~min .. ~max], in
      which case it returns the nearest end of the range. *)

  val between: low:t -> high:t -> t -> bool
  (** [between ~low ~high t] returns [true] iff [t] is inside the open-open range [~low .. ~high].
  *)
end
...
module type SMonoZero = sig
  ...
  (** [is_positive t] returns [true] if [t] is positive, [false] otherwise. NB: Positive does {i
      not} include zero. *)
  ...
  (** [is_non_positive t] returns [false] if [t] is positive, [true] otherwise. NB: Positive does {i
      not} include zero. *)
...
(** Key interface required when creating a comparator (see {!module:Cmper}) in order to use a type
    as a key. Only monomorphic keys are supported by the keyed container types, so there is no need
    for a polymorphic version of this module. *)
module type Key = sig
  type t
  (** Type being hashed/compared. *)

  val hash_fold: t -> Hash.State.t -> Hash.State.t
  (** [hash_fold a state] incorporates the hash of [a] into [state] and returns the resulting state.
  *)
...
(** Functor output signature for infix comparisons on comparable polymorphic types, e.g. ['a array].
*)
...
(** Functor input interface for comparable polymorphic types, e.g. [('a, 'cmp) Ordset]. *)
...
(** Functor output signature for infix comparisons on comparable polymorphic types, e.g. [('a, 'cmp)
    Ordset]. *)
...
(** Functor input interface for comparable polymorphic types, e.g. [('k, 'v, 'cmp) Ordmap]. *)
...
(** Functor output signature for infix comparisons on comparable polymorphic types, e.g. [('k, 'v,
    'cmp) Ordmap]. *)
