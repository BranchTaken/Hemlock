...
val close: t -> Error.t option
(** [close t] closes the external mutable Unix file descriptor associated with
    [t] and returns None or an error if it could not be closed. *)
...
val seek: sint -> t -> (uns, Error.t) result
(** [seek i t] seeks the external mutable Unix file descriptor associated with
    [t] to point to the [i]th byte relative to the current byte position of the
    file. Returns the new byte index relative to the beginning of the file or an
    error if it could not be changed. *)

(* val seek_hlt: sint -> t $-> uns *)
val seek_hlt: sint -> t -> uns
(** [seek_hlt i t] seeks the external mutable Unix file descriptor associated
    with [t] to point to the [i]th byte relative to the current byte position of
    the file. Returns the new byte index relative to the beginning of the file
    or halts if it could not be changed. *)

(* val seek_hd: sint -> t $-> (uns, Error.t) result *)
val seek_hd: sint -> t -> (uns, Error.t) result
(** [seek_hd i t] seeks the external mutable Unix file descriptor associated
    with [t] to point to the [i]th byte relative to the head of the file.
    Returns the new byte index relative to the beginning of the file or an error
    if it could not be changed. *)

(* val seek_hd_hlt: sint -> t $-> uns *)
val seek_hd_hlt: sint -> t -> uns
(** [seek_hd_hlt i t] seeks the external mutable Unix file descriptor associated
    with [t] to point to the [i]th byte relative to the head of the file.
    Returns the new byte index relative to the beginning of the file or halts if
    it could not be changed. *)

(* val seek_tl: sint -> t $-> (uns, Error.t) result *)
val seek_tl: sint -> t -> (uns, Error.t) result
(** [seek_tl i t] seeks the external mutable Unix file descriptor associated
    with [t] to point to the [i]th byte relative to the tail of the file.
    Returns the new byte index relative to the beginning of the file or an error
    if it could not be changed. *)

(* val seek_tl_hlt: ?i:sint -> t $-> uns *)
val seek_tl_hlt: sint -> t -> uns
(** [seek_tl_hlt i t] seeks the external mutable Unix file descriptor associated
    with [t] to point to the [i]th byte relative to the tail of the file.
    Returns the new byte index relative to the beginning of the file or halts if
    it could not be changed. *)
...
module Stream : sig
  ...
  (** [write file t] takes an open [file] with write permissions and writes, in
      order, all buffers from [t] to it. Returns an error if not all bytes could
      be written. *)

  (* val write_hlt: file -> /t $t $-> unit *)
  val write_hlt: file -> t -> unit
  (** [write_hlt file t] takes an open [file] with write permissions and writes,
      in order, all buffers from [t] to it. Halts if not all bytes could be
      written. *)
end
