...
val debias: t -> t
(* [debias t] creates an unbiased version of [t]. *)
...
module Cursor : sig
  ...
  (** [bias source pred t] creates a biased version of [t] with [source]'s bias, and [pred] as the
      predecessor cursor. When moving backwards to the location of [pred] the cursor reverts to
      [pred]. This makes it possible to move backwards and always recover the bias of predecessor
      cursors. *)

  val debias: t -> t
  (* [debias t] creates an unbiased version of [t]. *)

  val bias_pred: t -> t option
  (* [bias_pred t] returns the [pred] cursor which was specified in [bias source pred t]. *)
...
module Slice : sig
  ...
  (** [line_context t] creates an expanded slice which contains the entirety of the line(s) on
      ...
      and the beginning line context extends through a source boundary, the base source is
      reported as being the top-level source (i.e. source directives are ignored). Similarly, if
      the ending line context extends past the input slice the past source is unconditionally
      reported as being the top-level source. This behavior avoids false source origin claims. *)
