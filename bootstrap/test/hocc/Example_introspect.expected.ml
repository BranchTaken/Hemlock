...
let introspect () =
    ...
    File.Fmt.stdout
      ...
      ~f:(fun Spec.State.{lr1ItemsetClosure; _} ->
        let Spec.Lr1ItemsetClosure.{index; added; _} = lr1ItemsetClosure in
        (index, added)
      ) Spec.states)
      |> Fmt.fmt "\n"
      |> ignore
