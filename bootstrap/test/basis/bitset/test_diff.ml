...
let test () =
  let test ms0 ms1 = begin
    ...
    let bitset = diff bitset0 bitset1 in
    let ms = to_list bitset in
    List.iter ms0 ~f:(fun m -> assert ((mem m bitset) || (mem m bitset1)));
    List.iter ms1 ~f:(fun m -> assert (not (mem m bitset)));
    List.iter ms ~f:(fun m ->
      assert ((mem m bitset0) && (not (mem m bitset1))));
  end in
  let test_disjoint ms0 ms1 = begin
    let bitset0 = of_list ms0 in
    let bitset1 = of_list ms1 in
    let bitset = diff bitset0 bitset1 in
    assert ((length bitset) = (length bitset0));
  end in
  let test_lists = [
    [];
    [0L];
